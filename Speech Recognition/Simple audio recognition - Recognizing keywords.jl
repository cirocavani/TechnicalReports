...
begin
	# create Normalization function at Main module (related to serialization)
	Core.eval(Main, Meta.parse("""
begin
	struct Normalization
		μ::Float32
		σ::Float32
		Normalization(μ, σ) = new(μ::Float32, iszero(σ) ? eps(Float32) : σ::Float32)
	end
	(f::Normalization)(x::AbstractArray{Float32}) = @.((x - f.μ) / f.σ)
end
"""))
	using .Main: Normalization
end
...
# ╔═╡ 4a191e84-2ba6-42c9-9d76-b214fd40f015
begin
	spectrogram(x) = unsqueeze(get_spectrogram(x); dims = 3)

	x_out = spectrogram(dataset_X[1])
	@assert size(x_out) == (129, 124, 1)

	x_out
end

# ╔═╡ b756ab36-1178-4748-b10c-62fc6b6293db
dataset_X_spec = mapobs(spectrogram, dataset_X);
...
begin
	function normalization_params(data)::Tuple{Float32,Float32}
		# data -> Vector{<:AbstractArray{Float32}}
		# for all i, length(data[i]) = K where K is a constant.
		# @assert length(Set(length.(data))) == 1
		μ = mean(mean.(data))::Float32
		σ = sqrt(mean(var.(data; mean = μ, corrected = false)))::Float32
		return μ, σ
	end
...
model = Chain(
	...
	Normalization(μ_train, σ_train),      # 32x32x1 -> 32x32x1
...
# ╔═╡ 40c06995-4d13-4080-9f4c-80804db6e545
julia_executable = joinpath(Sys.BINDIR, "julia")

# ╔═╡ e8ce59b4-b36f-4705-b62c-f36a1a812f1f
isfile(julia_executable)

# ╔═╡ e1b562c3-5787-403a-bcd9-76ef4e10312e
...
let
	...
	wav_file = rand(dataset_files)
	run(`$julia_executable --project=$pluto_project $inference_app_file $model_file $wav_file`)
end;
...
manifest_format = "2.0"
project_hash = "d15ebb2f2fbd38690c7e85dc2667f3d46b6167bf"
...
git-tree-sha1 = "c56c199dcd825c289b32a2c526f0d5855c2405e2"
...
git-tree-sha1 = "82f9c8a13aa676bdd85e9a69c4597856f93abeef"
